<?php
/*
 * Template Name: Coaching Booking Payment Template
	...
	if(empty($booking_id)){
 	...
 	if(isset($_POST['action']) && $_POST['action'] == 'booking-payment'){
		...
		if(!empty($booking_id)){
			...
			$start_end_time = date("y-m-d h:i A", strtotime($booking_dates->session_date." ".$booking_dates->from_time));
			$from_time = date("y-m-d h:i A", strtotime($booking_dates->session_date." ".$booking_dates->from_time));
			$to_time = date("y-m-d h:i A", strtotime($booking_dates->session_date." ".$booking_dates->to_time));
			$duration = dateDiff($from_time, $to_time);

			//XDk_dc_session_booking_dates
			$sql_main_slot_id = "SELECT booking_main_slot_id FROM XDk_dc_session_booking_dates WHERE booking_id = '$booking_id'";
			$booking_main_slot_id = getRow($sql_main_slot_id);
			$booked_slot_id = $booking_main_slot_id[0]['booking_main_slot_id'];

			$sql_session_all_dates = "SELECT * FROM XDk_dc_session_booking_dates WHERE booking_id = '$booking_id' AND booking_main_slot_id = '$booked_slot_id'";
			$get_booked_all_slots = getRow($sql_session_all_dates);
			//print_r($get_booked_all_slots); die;

			$sql_session_dates = "SELECT topic FROM XDk_dc_session_dates WHERE id = '$booked_slot_id'";
			$booked_slot_topic = getRow($sql_session_dates);
			$booked_slot_topic_name = $booked_slot_topic[0]['topic'];
			$zoom_meeting_topic = !empty($booked_slot_topic_name) ? trim($booked_slot_topic_name) : 'FullyBossed Zoom Meeting';

			$duration = 0;
			foreach($get_booked_all_slots as $val_gb) {
				$to_time = date("y-m-d h:i A", strtotime($val_gb['session_date']." ".$val_gb['to_time']));
				$duration = $duration + dateDiff($from_time, $to_time);
			}
			...
			foreach($get_booked_all_slots as $val_gb) {
				...
				$Save_Meet_Data_arr['start_time'] = $val_gb['from_time'];
			...
			$url = home_url().'/coaching-booking-payment-details/?booking-id='.base64_encode($booking_id);
			echo '<script>
			location.assign("'.$url.'");
			</script>';
			exit;
